from rest_framework import serializers
from django.contrib.auth import get_user_model
from .models import LenderProfile, BorrowerProfile

User = get_user_model()


class RegisterSerializer(serializers.ModelSerializer):
    password = serializers.CharField(write_only=True, min_length=8)
    password2 = serializers.CharField(write_only=True)

    class Meta:
        model = User
        fields = ('email', 'full_name', 'phone', 'password', 'password2')

    def validate(self, data):
        if data['password'] != data['password2']:
            raise serializers.ValidationError({'password': 'Passwords do not match.'})
        return data

    def create(self, validated_data):
        validated_data.pop('password2')
        user = User.objects.create_user(**validated_data)
        LenderProfile.objects.create(user=user)
        BorrowerProfile.objects.create(user=user)
        return user


class UserSerializer(serializers.ModelSerializer):
    reputation_score = serializers.ReadOnlyField()
    total_rentals = serializers.ReadOnlyField()
    total_listings = serializers.ReadOnlyField()

    class Meta:
        model = User
        fields = (
            'id', 'email', 'full_name', 'phone', 'avatar', 'bio',
            'location', 'latitude', 'longitude', 'date_joined',
            'is_verified', 'reputation_score', 'total_rentals', 'total_listings'
        )
        read_only_fields = ('id', 'email', 'date_joined', 'is_verified')


class UserPublicSerializer(serializers.ModelSerializer):
    """Minimal public profile — shown to other users."""
    reputation_score = serializers.ReadOnlyField()
    total_rentals = serializers.ReadOnlyField()
    total_listings = serializers.ReadOnlyField()

    class Meta:
        model = User
        fields = (
            'id', 'full_name', 'avatar', 'bio', 'location',
            'date_joined', 'is_verified', 'reputation_score',
            'total_rentals', 'total_listings'
        )


class LenderProfileSerializer(serializers.ModelSerializer):
    class Meta:
        model = LenderProfile
        fields = ('preferred_payment', 'auto_approve', 'total_earned')
        read_only_fields = ('total_earned',)


class BorrowerProfileSerializer(serializers.ModelSerializer):
    class Meta:
        model = BorrowerProfile
        fields = ('preferred_categories', 'total_spent', 'is_trusted')
        read_only_fields = ('total_spent', 'is_trusted')
